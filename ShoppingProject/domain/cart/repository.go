...
// 返回购物车中所有item
func (r *ItemRepository) GetItems(UserId uint) ([]Item, error) {
	var cartItems []Item
	var cart Cart
	r.db.Model(Cart{}).Where("UserID=?", UserId).First(&cart)
	err := r.db.Where(&Item{CartID: cart.ID}).Find(&cartItems).Error
	if err != nil {
		return nil, err
	}
